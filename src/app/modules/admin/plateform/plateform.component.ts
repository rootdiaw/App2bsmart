...
export class PlateformComponent implements OnInit {
  ...
  displayedColumns = [ 'reseau', 'heure_post', 'format'];
  dataSource = ELEMENT_DATA;
  constructor(@Inject(MAT_DIALOG_DATA) public data: {var_objectifmacrom: String}) 
  {
  ...
  ngOnInit(): void {
    ...
    let h="Linkedin";
     this.var_objectifmacrom1.forEach((element: any) =>{
      if(element=="Notoriété"){
         this.tab.push(a,b,f)
           }
        if((element=="Création de trafic")||(element=="Conversion")){
         this.tab.push(a,b,c)
          }
       if((element=="Relactionnel")||(element=="Connexion avec des Leaders d\'opinion")||(element=="Relations publiques")){
         this.tab.push(e,h)
        }
      if(element=="Personal branding (B2B)"){
         this.tab.push(a,e,h)
         }
       if(element=="Recrutement"){
         this.tab.push(a,b,h)
         }
       if((element=="Capital sympathie")||(element=="Création de communauté")){
         this.tab.push(a,d,f,g)
          }
      if(element=="Didactique"){
         this.tab.push(b,c)
          }     
    }) 
    this.filteredArray = this.tab.filter( (ele,pos)=>this.tab.indexOf(ele) == pos);
    console.log("mon tableau",this.filteredArray)
  }
